...
/**
 * Saisie des donn�es du client + affichage des informations saisies pour validation avec redirection vers la passerelle de paiement
 */
require 'header.php';
...
// Passage de commandes r�serv� aux utilisateurs enregistr�s
...
switch ($op) {
	// ****************************************************************************************************************
	case 'default': // Pr�sentation du formulaire
	                 // ****************************************************************************************************************
	...
	// ****************************************************************************************************************
	case 'gateway': // Validation finale avant envoi sur la passerelle de paiement (ou arr�t)
	                 // ****************************************************************************************************************
		if ($h_oledrion_caddy->isCartEmpty()) {
			oledrion_utils::redirect(_OLEDRION_CART_IS_EMPTY, OLEDRION_URL, 4);
		}
		...
		foreach ($cartForTemplate as $line) {
			...
			$panier->setVar('caddy_pass', md5(xoops_makepass())); // Pour le t�l�chargement
		...
		// Pr�sentation du formulaire pour envoi � la passerelle de paiement
		// Pr�sentation finale avec panier en variables cach�es ******************************
		$registry = new oledrion_registryfile();
...
$xoopsTpl->assign('breadcrumb', oledrion_utils::breadcrumb(array(
	OLEDRION_URL . basename(__FILE__) => _OLEDRION_VALIDATE_CMD)));

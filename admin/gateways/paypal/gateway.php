...
class oledrion_paypal extends oledrion_gateway {
	...
	/**
	 * Retourne le formulaire utilis� pour param�trer la passerelle de paiement
	 *
	...
	/**
	 * Sauvegarde des param�tres de la passerelle de paiement
	 *
	 * @param array $data Les donn�es du formulaire
	 * @return boolean Le r�sultat de l'enregistrement des donn�es
	 */
	...
	/**
	 * Retourne les �l�ments � ajouter au formulaire en tant que zones cach�es
	 *
	...
	function getCheckoutFormContent($order) {
		...
		$ret['return'] = OLEDRION_URL . 'thankyou.php'; // Page (g�n�rique) de remerciement apr�s paiement
		$ret['image_url'] = ICMS_URL . '/images/logo.gif';
		$ret['cpp_header_image'] = ICMS_URL . '/images/logo.gif';
	...
	/**
	 * Utilis�e lors du dialog avec Paypal dans le cas de l'utilisation de l'IPN
	 * Note : Sp�cifique Paypal
	 *
	 ...
	 * L'appellant se charge de v�rifier que le fichier log existe
	 *
	 * @param string $gatewaysLogPath Le chemin d'acc�s complet au fichier log
	 * @return void
	 */
		...
		if ($fp) {
			...
			while (!feof($fp)) {
				...
				if (strcmp($res, "VERIFIED") == 0) {
					...
					if ($paypalok) {
						$ref = intval($_POST['custom']); // Num�ro de la commande
						$commande = null;
						$commande = $this->handlers->h_oledrion_commands->get($ref);
						if (is_object($commande)) {
							if ($montant == $commande->getVar('cmd_total')) { // Commande v�rifi�e

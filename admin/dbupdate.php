...
$wasUpdated = false;
// Pr�sence des nouvelles tables et nouvelles zones dans la base de donn�es
...
/**
 * Augmentation des types num�raires pour accepter le million
 *
	...
	if (xoops_trim($definition['Type']) == 'decimal(7,2)') {
		$tablesToUpdates = array(
			'oledrion_products' => array(
				...
				'product_ecotaxe'),
			'oledrion_caddy' => array(
				'caddy_price'),
			'oledrion_commands' => array(
				'cmd_shipping'),
...
if ($wasUpdated) {
	$op = 'maintain'; // On force la mise � jour du cache de requ�tes
}
?>
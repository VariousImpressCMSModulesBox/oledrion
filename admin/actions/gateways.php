...
switch ($action) {
	// ****************************************************************************************************************
	case 'default': // Liste des passerelles de paiement install�s
	                 // ****************************************************************************************************************
		xoops_cp_header();
		...
		foreach ($installedGateways as $installedGateway) {
			if (oledrion_gateways::gatewayClassFileExists($installedGateway)) { // Il y a une classe donc c'est bon
				if (!oledrion_gateways::loadGatewaysLanguageDefines($installedGateway)) { // On n'a pas r�ussi � charger le fichier de traduction
					continue;
				}
	...
	// ****************************************************************************************************************
	case 'setDefaultGateway': // Choix de la passerelle de paiement par d�faut
	                           // ****************************************************************************************************************
		xoops_cp_header();
		oledrion_adminMenu(12);
		$opRedirect = '?op=gateways';
	...
	// ****************************************************************************************************************
	case 'parameters': // Param�tres de la passerelle de paiement s�lectionn�e
	                    // ****************************************************************************************************************
		xoops_cp_header();
		oledrion_adminMenu(12);
		oledrion_utils::htitle(_AM_OLEDRION_INSTALLED_GATEWAYS, 4);
	...
	// ****************************************************************************************************************
	case 'saveparameters': // Enregistrement des param�tres de la passerelle de paiement
	                        // ****************************************************************************************************************
		xoops_cp_header();
		oledrion_adminMenu(12);
		$opRedirect = '?op=gateways';
		...
		if (oledrion_gateways::gatewayClassFileExists($gateway)) {
			if (!oledrion_gateways::loadGatewaysLanguageDefines($gateway)) { // Le chargement des traductions a �chou�
				oledrion_utils::redirect(_AM_OLEDRION_GATEWAYS_ERROR2, $baseurl . $opRedirect, 4);
			}
			oledrion_gateways::includeGatewayClass($gateway);
			if (oledrion_gateways::gatewayClassExists($gateway)) {

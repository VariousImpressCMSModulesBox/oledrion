...
switch ($action) {
	...
	case 'default': // Gestion des produits
		...
		$itemsCount = $h_oledrion_products->getCount($criteria); // Recherche du nombre total de produits r�pondants aux crit�res
		...
		$onlineSelect = oledrion_utils::htmlSelect('filter_product_online', array(
			2 => _YES,
			1 => _NO), $filter_product_online);
		$recommendedSelect = oledrion_utils::htmlSelect('filter_product_recommended', array(
			1 => _YES,
			2 => _NO), $filter_product_recommended);
		...
		foreach ($products as $item) {
			...
			$recommended = '';
			if ($item->isRecommended()) { // Si le produit est recommand�, on affiche le lien qui permet d'arr�ter de le recommander
	...
	// ****************************************************************************************************************
	case 'unrecommend': // Arr�ter de recommander un produit
	                     // ****************************************************************************************************************
	...
	case 'edit': // Edition d'un produit
		...
		if (!oledrion_utils::getModuleOption('create_thumbs')) { // L'utilisateur se charge de cr�er la vignette lui m�me
		...
		// Recherche de tous les produits sauf celui-l�
		...
		// Recherche des produits relatifs � ce produit
		if ($edit) {
			$criteria = new CriteriaCompo();
		...
		if (oledrion_utils::getModuleOption('use_tags')) {
			require_once ICMS_ROOT_PATH . '/modules/tag/include/formtag.php';
	...
	case 'saveedit': // Sauvegarde des informations d'un produit
		...
		} else { // Il faut cr�er la vignette pour l'utilisateur
			$indiceAttached = 1;
		...
		// T�l�chargement du fichier attach�
		$destname = '';
		...
		if ($res) {
			...
			if ($add == true) {
				...
				$plugins->fireAction(oledrion_plugins::EVENT_ON_PRODUCT_CREATE, new oledrion_parameters(array(
					'product' => $item)));
			}
	...
	case 'confdelete': // Confirmation de la suppression d'un produit
		...
		xoops_confirm(array(
			'op' => 'products',
			'action' => 'delete',
			'id' => $id), 'index.php', _AM_OLEDRION_CONF_DELITEM . '<br />' . $item->getVar('product_title'));
		break;

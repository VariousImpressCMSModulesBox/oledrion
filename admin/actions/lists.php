...
switch ($action) {
	...
	case 'default': // Liste des listes
		...
		$items = $oledrion_handlers->h_oledrion_lists->getRecentLists(new oledrion_parameters(array(
			'start' => $start,
			'limit' => $limit)));
		if (count($items) > 0) {
		...
		foreach ($items as $item) {
			...
			echo "<td align='center'><a target='_blank' href='" . ICMS_URL . '/userinfo.php?uid=' . $item->list_uid . "'>" . $userName . "</td>";

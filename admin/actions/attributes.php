...
switch ($action) {
	...
	case 'default': // Liste des attributs produits
		...
		$typeSelect = oledrion_utils::htmlSelect('filter_attribute_type', array(
			...
			OLEDRION_ATTRIBUTE_SELECT => _AM_OLEDRION_TYPE_LIST), $filter_attribute_type);
	...
	case 'edit': // Edition d'un attribut
		...
		$productsSelect = $oledrion_handlers->h_oledrion_products->productSelector(new oledrion_parameters(array(
		...
		// Param�trage (pour les boutons radio et cases � cocher, le d�limiteur, pour les listes d�roulantes, le nombre d'�l�ments visibles et la s�lection multiple)
		// Les boutons radio et cases � cocher
	...
	case 'ajaxoptions': // Traitement en Ajax des options
		...
		if (!isset($_SESSION['oledrion_attribute'])) {
			if ($attribute_id == 0) { // Cr�ation, rajouter une zone
		...
		if (isset($_POST['subaction'])) {
			switch (xoops_trim(strtolower($_POST['subaction']))) {
				...
				case 'add': // Ajout d'une option vide (� la fin)
					$attribute->addEmptyOption();
					break;

				case 'up': // D�placement d'une option vers le haut
					$option = isset($_POST['option']) ? intval($_POST['option']) : false;
					if ($option !== false) {
				...
				case 'down': // D�placement d'une option vers le haut
					$option = isset($_POST['option']) ? intval($_POST['option']) : false;
					if ($option !== false) {

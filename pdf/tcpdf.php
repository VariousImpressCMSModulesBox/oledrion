...
if (!class_exists('TCPDF', false)) {
	...
	class TCPDF {
		 ...
		 * It will be substituted as the document is closed.<br />
		 *
		 ...
		 */
		public function Annotation($x, $y, $w, $h, $text, $opt = array(
			'Subtype' => 'Text'), $spaces = 0) {
		...
		public function CoonsPatchMesh($x, $y, $w, $h, $col1 = array(), $col2 = array(), $col3 = array(), $col4 = array(), $coords = array(
			...
			0.67,
			0.00,
			0.33), $coords_min = 0, $coords_max = 1) {
			$this->Clip($x, $y, $w, $h);
		...
		public function writeHTML($html, $ln = true, $fill = false, $reseth = false, $cell = false, $align = '') {
			...
			while ($key < $maxel) {
				if ($dom[$key]['tag'] or ($key == 0)) {
					...
					if ((!$this->newline) and ($dom[$key]['value'] == 'img') and (isset($dom[$key]['attribute']['height'])) and ($dom[$key]['attribute']['height'] > 0) and (!((($this->y + $this->pixelsToUnits($dom[$key]['attribute']['height'])) > $this->PageBreakTrigger) and empty($this->InFooter) and $this->AcceptPageBreak()))) {
				...
				case 'img':
					{
						if (isset($tag['attribute']['src'])) {
							...
							$tag['attribute']['src'] = str_replace(K_PATH_URL, ICMS_ROOT_PATH . '/modules/', $tag['attribute']['src']);

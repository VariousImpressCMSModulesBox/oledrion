...
function ReadUFM($file, &$cidtogidmap) {
	...
	foreach ($a as $l) {
		...
		if ($code == 'U') {
			...
			if ($cc != -1) {
				...
				$glyph = $e[10];
				$widths[$cc] = $w;
				if ($cc == ord('X')) $fm['CapXHeight'] = $e[13];
			...
			if ($gn == '.notdef' && !isset($fm['MissingWidth'])) $fm['MissingWidth'] = $w;
	...
	if (!isset($fm['MissingWidth'])) $fm['MissingWidth'] = 600;
...
function MakeFont($fontfile, $ufmfile) {
	...
	if (!file_exists($ufmfile)) die('<B>Error:</B> UFM file not found: ' . $ufmfile);
	$cidtogidmap = '';
	...
	if ($fontfile) {
		...
		if ($type != 'TrueTypeUnicode') die('<B>Error:</B> incorrect font type: ' . $type);
	}
	// Start generation
	$s = '<?php' . "\n";
	$s .= '$type=\'' . $type . "';\n";

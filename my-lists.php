...
if ($uid == 0) {
	oledrion_utils::redirect(_OLEDRION_ERROR23, ICMS_URL . '/register.php', 4);
}
...
function listForm($op, $product_id = 0) {
	...
	if ($op == 'edit') {
		...
		// V�rification, est-ce que l'utilisateur courant est bien le propri�taire de cette liste ?
...
switch ($op) {
	...
	// ************************************************************************
	case 'addProduct': // Ajout d'un produit � une liste *********************
	                    // ************************************************************************
		$xoopsTpl->assign('op', $op);
	...
	// ************************************************************************
	case 'addProductToList': // Ajout d'un produit � une liste, s�lection de la liste
	                          // ************************************************************************
		$xoopsTpl->assign('op', $op);
		...
		if ($list_id == 0) { // Ajouter � une nouvelle liste
			$sform = listForm('addList', $product_id);
			$title = _OLEDRION_ADD_LIST;
			$breadcrumb = array(
				OLEDRION_URL . 'all-lists.php' => _MI_OLEDRION_SMNAME11,
		...
		} else { // Ajouter � une liste existante
			if (!$handlers->h_oledrion_lists->isThisMyList($list_id)) {
				oledrion_utils::redirect(_OLEDRION_ERROR25, $baseurl, 8);
			}
	...
	case 'delete': // Suppression d'une liste ********************************
		...
		xoops_confirm(array(
			'op' => 'reallyDelete',
			'list_id' => $list_id), $baseurl, _OLEDRION_DELETE_LIST . '<br />' . $item->getVar('list_title'));
		break;
		...
		if ($res) {
			if (isset($_POST['product_id'])) {
				...
				if ($product_id > 0) {
					$product = null;
					$product = $handlers->h_oledrion_products->get($product_id);
					if (is_object($product) && $product->isProductVisible()) { // On peut ajouter le produit � cette nouvelle liste
						$res = $handlers->h_oledrion_products_list->addProductToUserList($item->getVar('list_id'), $product_id);
						if ($res) { // Mise � jour du nombre de produits de la liste

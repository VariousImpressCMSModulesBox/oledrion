...
function listCart() {
	...
	$xoopsTpl->assign('discountsCount', $discountsCount); // Nombre de r�ductions appliqu�es
	...
	case 'addproduct': // Ajout d'un produit
		...
		if ($product->getVar('product_stock') - 1 >= 0) {
			...
			if ($product->productAttributesCount() > 0) { // Si le produit a des attributs
				...
				// On commence par v�rifier que les attributs obligatoires sont renseign�s
				...
				// Toujours l� c'est que le produit a des attributs et qu'ils sont renseign�s
				...
				foreach ($productAttributes as $attribute) {
					...
					} else { // On va chercher sa valeur par d�faut
...
$xoopsTpl->assign('breadcrumb', oledrion_utils::breadcrumb(array(
	OLEDRION_URL . basename(__FILE__) => _MI_OLEDRION_SMNAME1)));

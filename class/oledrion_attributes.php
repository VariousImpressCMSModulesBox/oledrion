...
// Le s�parateur de donn�es utilis� en interne
define("OLEDRION_ATTRIBUTE_SEPARATOR", '|');
define("OLEDRION_EMPTY_OPTION", '');

// Le s�parateur de ligne lorsque l'option est un bouton radio ou des cases � cocher
define("OLEDRION_ATTRIBUTE_CHECKBOX_WHITE_SPACE", 1); // S�parateur de ligne = espace blanc
define("OLEDRION_ATTRIBUTE_CHECKBOX_NEW_LINE", 2); // S�parateur de ligne = retour � la ligne

// Les options par d�faut lorsque l'option est une liste d�roulante
define("OLEDRION_ATTRIBUTE_SELECT_VISIBLE_OPTIONS", 1); // Valeur par d�faut, nombre d'options visibles
define("OLEDRION_ATTRIBUTE_SELECT_MULTIPLE", false);

// Valeur par d�faut, s�lecteur multiple ?
class oledrion_attributes extends Oledrion_Object {
	...
	/**
	 * Indique si l'attribut courant a une valeur par d�faut
	 *
	 * @return boolean
	...
	/**
	 * Ajoute une option vide � la fin (avec des valeurs par d�faut)
	 *
	 * @return boolean
	 * @since 2.3.2009.03.12
	 */
	...
	/**
	 * R�initialisation des options de l'attribut
	 *
	 * @return boolean True
	 * @since 2.3.2009.03.10
	 */
	 ...
	 *
	 * @param integer $optionNumber (de 0 � N)
	 ...
	 * @return boolean True si la mise � jour s'est faite sinon false
	 * @since 2.3.2009.03.10
	 */
	...
	/**
	 * Fonction interne charg�e du d�placement d'une option soit vers le haut soit vers le bas
	 *
	...
	/**
	 * D�place une option vers le haut
	 *
	 * @param integer $optionNumber
	 * @return boolean
	 * @since 2.3.2009.03.10
	...
	/**
	 * D�place une option vers le bas
	 *
	 * @param integer $optionNumber
	 * @return boolean
	 * @since 2.3.2009.03.10
	...
	/**
	 * Retourne le prix de l'attribut par d�faut
	 *
	 * @return float
	...
	function getDefaultAttributePrice($format = 'e') {
		...
		if ($defaultValue != '') { // Il y a une option par d�faut donc un prix
			$values = $this->getOption('attribute_values', $format);
			$prices = $this->getOption('attribute_prices', $format);
			$counter = 0;
			if (count($values) > 0) {
	...
	/**
	 * Retourne la valeur par d�faut de l'attribut courant
	 *
	 * @param string $format
	 * @return string
	 * @since 2.3.2009.03.20
	...
	/**
	 * Retourne une liste combin�e des options de l'attribut
	 *
	 * @param string $format Format dans lequel renvoyer les donn�es
	 * @param boolean $withFormatedPrices Faut il retourner les prix format�s ?
	...
	/**
	 * Retourne le type de l'attribut courant (son libell�)
	 *
	...
	/**
	 * Retourne le libell� d'une option en fonction de son nom
	 *
	 * @param string $optionName
	 * @return string
	 */
	...
	/**
	 * Cr�ation du code html de l'attribut
	 *
	 * On utilise le contenu de templates html (r�alis�s en Smarty) pour cr�er le contenu de l'attribut
	 * Templates utilis�s (selon le type d'attribut) :
	...
	function render(oledrion_products $product) {
		...
		if ($isInCart === false) { // Le produit n'est pas dans le panier, on prend la valeur par d�faut
			$defaultValue = array(
				$this->getVar('attribute_default_value'));
		} else { // Le produit est dans le panier, on va chercher les options qui sont s�lectionn�es
			...
			} else { // On prend la valeur par d�faut
				if ($this->attribute_type == OLEDRION_ATTRIBUTE_RADIO) { // Pour les boutons radio, il ne peut y avoir qu'un �l�ment de s�lectionn�
		...
		switch ($this->getVar('attribute_type')) {
			case OLEDRION_ATTRIBUTE_SELECT: // Liste d�roulante
				$templateName = 'oledrion_attribute_select.html';
				$multiple = '';
				if ($option2 == 1) { // La s�lection multiple est autoris�e
			...
			case OLEDRION_ATTRIBUTE_CHECKBOX: // Cases � cocher
...
class OledrionOledrion_attributesHandler extends Oledrion_XoopsPersistableObjectHandler {
	 ...
	 *
	 * @param integer $product_id Le produit concern�
	 * @return array
	 */
	function getProductsAttributesList($product_id, $attributesIds = null) {
		...
		$criteria->setSort('attribute_weight, attribute_title'); // L'ajout du titre dans le tri permet de trier m�me lorsque le poids n'est pas valoris�
	 ...
	 *
	 * @param object $product Le produit concern�
	 ...
	 *
	 * @param oledrion_attributes $attribute L'attribute � cloner
	 * @return mixed Soit le nouvel attribut si tout a bien march� sinon false
	 * @since 2.3.2009.03.16
	 */
	...
	/**
	 * Retourne la liste des produits utilis�s dans la table (liste unique)
	 *
	...
	/**
	 * Suppression d'un attribut (et de ce qui y est rattach�)
	 *
	...
	/**
	 * Retourne le nom des champs (repr�sentant les attributs) obligatoires que l'on devrait trouver suite � une s�lection de produit
	 *
	 * @param oledrion_products $product
	...
	/**
	 * Calcul le prix HT des options s�lectionn�es pour un produit
	 *
	 * @param array $choosenAttributes [cl�] = attribute_id, [value] = array(valueId1, valueId2 ...)
	 * @param integer $product_vat_id L'ID de TVA du produit
	 * @param array $descriptions Tableau valoris� par la m�thode [cl�] = Id attribut [valeur] = array('attribute_title', array('attribute_names', 'attribute_prices'))
	 * @return float

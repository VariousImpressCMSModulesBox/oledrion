...
class Oledrion_Object extends XoopsObject {
	...
	// TODO: Rajouter une m�thode intsert() et delete()

	/**
	 * Permet de valoriser un champ de la table comme si c'�tait une propri�t� de la classe
	 *
	...
	/**
	 * Permet d'acc�der aux champs de la table comme � des propri�t�s de la classe
	 *
	 * @example echo $enregistrement->nom_du_champ;
	 *         
	 * @param string $key Le nom du champ que l'on souhaite r�cup�rer
	 * @return mixed
	 */
	function __get($key) {
...
class Oledrion_XoopsPersistableObjectHandler extends XoopsObjectHandler {
	...
	/**
	 * Retourne des �l�ments selon leur ID
	 *
	 * @param array $ids Les ID des �l�ments � retrouver
	 * @return array Tableau d'objets (cl� = id key name)
	 */
	...
	function compareObjects($old_object, $new_object) {
		...
		foreach ($vars_name as $one_var) {
			if ($old_object->getVar($one_var, 'f') == $new_object->getVar($one_var, 'f')) {} else {
				$ret[$one_var] = array(
					$old_object->getVar($one_var),
					$new_object->getVar($one_var));
			}
		}
		return $ret;
	}

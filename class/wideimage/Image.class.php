 ...
 */
if (!defined('WI_LIB_PATH')) define('WI_LIB_PATH', dirname(__FILE__) . DIRECTORY_SEPARATOR);
...
abstract class wiImage {
	...
	static function assertValidImageHandle($handle) {
		if (!self::isValidImageHandle($handle)) throw new wiInvalidImageHandleException("{$handle} is not a valid image handle.");
	}
	...
	/**
	 * Loads an image from a string, file, or a valid image handle.
	 * This function
	...
	static function load($source, $format = null) {
		...
		return call_user_func(array(
			'wiImage',
			'loadFrom' . $predictedSourceType), $source, $format);
	}

	/**
	 * Create and load an image from a file or URL.
	...
	static function loadFromFile($uri, $format = null) {
		...
		if (!self::isValidImageHandle($handle)) throw new wiInvalidImageSourceException("File '{$uri}' appears to be an invalid image source.");

		return self::loadFromHandle($handle);
	}

	/**
	 * Create and load an image from a string.
	...
	static function loadFromString($string) {
		...
		if (!self::isValidImageHandle($handle)) throw new wiInvalidImageSourceException("String doesn't contain valid image data.");
	...
	static function loadFromHandle($handle) {
		if (!self::isValidImageHandle($handle)) throw new wiInvalidImageSourceException("Handle is not a valid GD image resource.");
	...
	/**
	 * Returns closest color index that matches the given RGB value.
	 * Uses
	...
	/**
	 * Returns the color index that exactly matches the given RGB value.
	 * Uses
	...
	/**
	 * Copies transparency information from $sourceImage.
	 * Optionally fills
	...
	function getCanvas() {
		if ($this->canvas == null) $this->canvas = new wiCanvas($this);
		return $this->canvas;
	}

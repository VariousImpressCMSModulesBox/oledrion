...
class oledrion_shelf {
	 ...
	 *
	 * @param string $type Le type de produits dont on veut r�cup�rer le nombre
	 */
	...
	function deleteProduct(oledrion_products $product) {
		...
		// Suppression dans les paniers persistants enregistr�s
		$this->handlers->h_oledrion_persistent_cart->deleteProductForAllCarts($id);

		// Les attributs qui lui sont rattach�s
		...
		// Et le produit en lui m�me, � la fin
		return $this->handlers->h_oledrion_products->delete($product, true);
	}

	/**
	 * Cherche et retourne la liste de produits relatifs � une liste de produits
	 *
	 * @param array $productsIds La liste des produits dont on cherche les produits relatifs
	 * @return array Cl� = ID Produit, valeurs (deuxi�me dimension) = liste des produits relatifs
	 */
	...
	/**
	 * Retourne une liste de produits selon certains crit�res
	 *
	 * @param oledrion_shelf_parameters $parameters Les param�tres de filtrage
	 * @return array Tableau pr�t � �tre utilis� dans les templates
	 */
	function getProducts(oledrion_shelf_parameters $parameters) {
		...
		// On commence par r�cup�rer la liste des produits
		...
		// Recherche des Id des cat�gories et des vendeurs
		...
		// On r�cup�re la liste des personnes qui ont soumis les produits
		if ($withXoopsUser) {
			$users = oledrion_utils::getUsersFromIds($xoopsUsersIDs);
		}

		// Il faut r�cup�rer la liste des produits relatifs
		...
		// R�cup�ration des fabricants, des vendeurs et des cat�gories
		...
		foreach ($products as $product) {
			...
			// L'utilisateur Xoops (�ventuellement)
			...
			$tmp['product_count'] = $count; // Compteur pour les templates (pour g�rer les colonnes)
			$ret[] = $tmp;
			$count++ ;
		}

...
CREATE TABLE `oledrion_products` (
  ...
  `product_sku` varchar(60) NOT NULL COMMENT 'internal product identifier',
  ...
  `product_url` varchar(255) NOT NULL COMMENT 'URL to an external page',
  `product_image_url` varchar(255) NOT NULL COMMENT 'URL of the big image',
  `product_thumb_url` varchar(255) NOT NULL COMMENT 'URL of the thumbnail',
  `product_submitter` int(11) unsigned NOT NULL default '0',
  `product_online` tinyint(1) NOT NULL default '0',
  `product_date` varchar(255) NOT NULL COMMENT 'publication date of the product',
  `product_submitted` int(10) unsigned NOT NULL default '0' COMMENT 'Date when the product was submitted to the site',
  `product_hits` int(11) unsigned NOT NULL default '0' COMMENT 'Number of times the product page was viewed',
  ...
  `product_stock` mediumint(8) unsigned NOT NULL COMMENT 'Quantity of product available in stock',
  `product_alert_stock` mediumint(8) unsigned NOT NULL COMMENT 'Quantity of stock from when an alert needs to be sent',
  ...
  `product_download_url` varchar(255) NOT NULL COMMENT 'download URL',
...
CREATE TABLE `oledrion_commands` (
  ...
  `cmd_uid` int(10) unsigned NOT NULL COMMENT 'user ID',
  ...
  `cmd_bill` tinyint(1) unsigned NOT NULL default '0' COMMENT 'The client asked a paper invoice',
  `cmd_password` varchar(32) NOT NULL COMMENT 'Used to print invoices online',
...
CREATE TABLE `oledrion_related` (
  ...
  `related_product_id` int(10) unsigned NOT NULL COMMENT 'Id of the master product',
  `related_product_related` int(10) unsigned NOT NULL COMMENT 'Id of the product to show with the master',
...
CREATE TABLE `oledrion_discounts` (
  ...
  `disc_group` int(10) unsigned NOT NULL COMMENT 'ImpressCMS group concerned by the discount (0=all the groups)',
  `disc_cat_cid` int(10) unsigned NOT NULL COMMENT 'Category concerned by the discount (0=all the categories)',
  `disc_vendor_id` int(10) unsigned NOT NULL COMMENT 'When the discount concerns products from a vendor (0=all the vendors)',
  `disc_product_id` int(10) unsigned NOT NULL COMMENT 'When the discount concerns a product (0=all the products)',
  `disc_price_type` tinyint(1) unsigned NOT NULL COMMENT 'Type of discount (degressif, amount/percentage)',
  ...
  `disc_price_amount_amount` double(7,2) NOT NULL COMMENT 'Amount or percentage of discount on the price',
  `disc_price_amount_type` tinyint(1) unsigned NOT NULL COMMENT 'Percent or euros ?',
  `disc_price_amount_on` tinyint(1) unsigned NOT NULL COMMENT 'Product or basket?',
  `disc_price_case` tinyint(1) unsigned NOT NULL COMMENT 'In what case? (All cases, if it's the first purchase, if the product has never been acquired etc.)',
  `disc_price_case_qty_cond` tinyint(1) NOT NULL COMMENT 'Larger, smaller, equal',
  `disc_price_case_qty_value` mediumint(8) NOT NULL COMMENT 'Amount of product to test',
  ...
  `disc_date_from` int(10) unsigned NOT NULL COMMENT 'Start date of the promotion',
  `disc_date_to` int(10) unsigned NOT NULL COMMENT 'End date of the promotion',
...
CREATE TABLE `oledrion_gateways_options` (
  ...
  `option_gateway` varchar(50) NOT NULL COMMENT 'Payment provider name',
...
) ENGINE=InnoDB COMMENT='Register the payment provider preferences';

...
/**
 * Renvoie la liste des produits recommand�s
 */
function b_oledrion_recomm_show($options) {
	...
	if ($h_oledrion_products->getRecommendedCount() > $limit) { // Il y a plus de produits recommand�s dans la BDD que dans le bloc, on affiche donc un lien vers la page des produits recommand�s
		$block['showMore'] = true;
	}
	...
	if (count($products) > 0) {
		...
		return $block;
	} else { // Pas de produits recommand�s
		return false;
	}
}
